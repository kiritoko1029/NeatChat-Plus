...
const cn = {
  ...
  Chat: {
    ...
    WebSearch: {
      ConfigTitle: "网络搜索配置",
      Enable: "启用网络搜索",
      EnableSubTitle: "启用后可以在聊天中使用网络搜索功能，提供实时信息",
      MaxResults: "最大搜索结果数量",
      MaxResultsSubTitle: "每次搜索返回的最大结果数量（1-20）",
      SearxngUrl: "SearXNG服务地址",
      SearxngUrlSubTitle: "自定义SearXNG实例地址，留空使用环境变量配置",
      SearxngUrlSubTitleWithEnv: (url: string) => `环境变量已配置: ${url}`,
      SearxngUrlPlaceholder: "https://searx.example.com",
      SearxngUrlPlaceholderWithEnv: "留空使用环境变量",
      AiGenerateKeywords: "AI生成搜索关键词",
      AiGenerateKeywordsSubTitle: "让AI根据用户问题自动生成更精准的搜索关键词",
      DefaultCollapsed: "默认折叠搜索结果",
      DefaultCollapsedSubTitle:
        "在AI回复中默认折叠显示搜索结果，用户可点击展开查看",
      ConfigSaved: "网络搜索配置已保存",
      ConfigDisabled: "网络搜索已禁用",
      Enabled: "网络搜索 (已启用)",
      Searching: "🔍 正在搜索网络获取相关信息...",
      ResultsTitle: (count: number) => `🔍 网络搜索结果 (${count})`,
      NoResults: "未找到相关网络信息，以下回答基于AI已有知识。",
      SearchFailed: "🔍 网络搜索失败",
      SearchError: (error: string) => `搜索过程中发生错误: ${error}`,
      CommonInstances: "常用公共实例：",
      CheckingEnvConfig: "正在检查环境配置...",
    },
    Upload: {
      FilesUploaded: (count: number) => `已上传 ${count} 个文件`,
      ImagesUploaded: (count: number) => `已上传 ${count} 张图片`,
      MaxFilesReached: "最多只能上传5个文件",
      MaxImagesReached: "最多只能上传3张图片",
      FilesTooLarge: "文件过大，已截断",
      TextTooLong: "文本过长，已自动转换为文件附件",
      ReadFileFailed: "读取文件失败",
      UploadFailed: "上传失败",
      PasteAsAttachment: "已将长文本转为附件",
    },
    McpTools: {
      Title: "MCP工具",
      Loading: "加载中...",
      NoTools: "没有可用的MCP工具",
      AddServers: "点击这里添加MCP服务器",
      ToolCount: (count: number) => `MCP工具${count > 0 ? ` (${count})` : ""}`,
    },
    TokenInfo: {
